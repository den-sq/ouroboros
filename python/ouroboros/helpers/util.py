...
def unique_high_dup(ar, *bin_weights: np.ndarray,
                    return_index: bool = False, return_inverse: bool = False, return_counts: bool = False,
                    axis: int = 1, sorted: bool = False, last_axis_sort: bool = False):
    """ Get Unique Integer Values (slow if low duplicates)
        Currently limited to 2D arrays and sortying over the first axis, will tweak at some point.
    ...
    for col in ar if last_axis_sort else np.flip(ar, 0):
        col_lookup = np.nonzero(np.bincount(col))[0]     # Unique values in this row
        col_nbits = np.ceil(np.log2(len(col_lookup))).astype(rec_type)
        col_data.append((col_nbits, col.dtype, col_lookup))

        if col_nbits:
            # Searchsorted gives values of each item by indexing on unique vals
            result = (np.searchsorted(col_lookup, col).astype(rec_type) << next_shift) | result
            next_shift += col_nbits
            if next_shift > (rec_type.itemsize * 8):
                raise NotImplementedError(f"{rec_type} Too Small: {rec_type.itemsize * 8}|{next_shift}")
